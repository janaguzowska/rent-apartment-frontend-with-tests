...
const stepPath = (path: string) => (id: string) => `${reservationBasePath(id)}/${path}`;
...
export const getNextStepPath = (id: string, path: string) => RESERVATION_STEPS[getStepIndex(id, path) + 1].path(id);
export const getBackStepPath = (id: string, path: string) => RESERVATION_STEPS[getStepIndex(id, path) - 1].path(id);


export const isNextButtonEnabled = (id: string, path: string) => getStepIndex(id, path) < RESERVATION_STEPS.length - 1;
export const isBackButtonEnabled = (id: string, path: string) => getStepIndex(id, path) > 1;
