...
const ReviewsComponent = ({
  ...
  useEffect(() => {
    api
      .post<Review[]>('/review/search', undefined, { offerId })
  ...
  const handleSubmit = (evt: FormEvent) => {
    ...
    api
      .post<void>('/review/add', undefined, newReview)
        ...
        >
          ...
          <div className="reviews__button-wrapper">
            ...
            <button
              ...
              type="submit"
              disabled={!rating || text.trim().length < 50}
            >
              Submit
            </button>
          </div>
...
const mapStateToProps = (state: AppState) => ({
  reviews: state.reviewState.reviews.filter(
    (review) => review.offerId === state.offerState.currentOffer?.id,
  ),
