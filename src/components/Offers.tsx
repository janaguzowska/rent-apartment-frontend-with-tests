...
const sortedByPrice = (offers: Offer[], sortType?: SortType | null) =>
  !sortType
    ? offers
    : offers.toSorted((a: Offer, b: Offer) => {
        switch (sortType) {
  ...
  return (
    <section className="cities__places places">
      ...
      <b className="places__found">
        {filteredByCity(offers, urlParams.city).length} places to stay in{' '}
        {urlParams.city}
      </b>
      <OutsideClickHandler
        onOutsideClick={() => {
          ...
          {showSortOptions && (
            <ul className="places__options places__options--custom places__options--opened">
              ...
              <li
                ...
                tabIndex={0}
                onClick={() => setSortType(SortType.PriceAsc)}
              >
                Price: low to high
              </li>
              <li
                className="places__option"
                tabIndex={0}
                onClick={() => setSortType(SortType.PriceDesc)}
              >
                Price: high to low
              </li>
              <li
                className="places__option"
                tabIndex={0}
                onClick={() => setSortType(SortType.RatingDesc)}
              >
                Top rated first
              </li>
            </ul>
      ...
      <div className="cities__places-list places__list tabs__content">
        {sortedByPrice(filteredByCity(offers, urlParams.city), sortType).map(
          (currentOffer) => (
